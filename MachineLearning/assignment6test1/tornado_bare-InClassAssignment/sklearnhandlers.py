...
class UpdateModelForDatasetId(BaseHandler):
    def post(self):
        ...
        '''
        data = json.load(self.request.body.decode("utf-8"))
        dsid = data['dsid']
        knn_pa = data['knn_pa']
        svm_pa = data['svm_pa']
        ...
        if l:
            ...
            self.db.models.update({"dsid":dsid},
                                  {  "$set": {"model":Binary(bytes),"model_id":1}  },
                upsert=True)
            bytes = pickle.dumps(c2)
            self.db.models.update({"dsid":dsid},
                                     {  "$set": {"model":Binary(bytes),"model_id":2}  },
                                     upsert=True)
...
class PredictOneFromDatasetId(BaseHandler):
    def post(self):
        ...
        self.write_json({"prediction_SVC":str(predLabel_SVC),
                          "prediction_KNN":str(predLabel_KNN)
                        })
